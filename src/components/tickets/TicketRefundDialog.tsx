...
const TicketRefundDialog = ({
  ...
  const renderTicketOptions = () => {
    ...
    for (let i = 1; i <= ticketCount; i++) {
      const translatedText = t('tickets.refund.ticketNumber', {
        number: i
      });
      ...
      options.push(<div key={i} className="flex items-center space-x-2">
          <RadioGroupItem value={`ticket-${i}`} id={`refund-ticket-${i}`} aria-describedby={`refund-ticket-${i}-description`} className="bg-gray-700 hover:bg-gray-600" />
          <Label htmlFor={`refund-ticket-${i}`} className="text-white cursor-pointer" id={`refund-ticket-${i}-description`}>
      ...
      <DialogContent className="bg-gray-800 border-gray-700 text-white max-w-md">
        ...
        <div className="space-y-6 mt-4">
          ...
          <div className="space-y-3">
            ...
            <RadioGroup value={selectedTicket} onValueChange={setSelectedTicket} className="space-y-2" aria-label={t('tickets.refund.selectTicket')}>
              {renderTicketOptions()}
            </RadioGroup>
          </div>
          ...
          <div className="flex space-x-3 pt-4">
            ...
            <Button onClick={handleRefund} disabled={!selectedTicket} className="flex-1 bg-gradient-to-r from-red-500 to-pink-500 hover:from-red-600 hover:to-pink-600">
