...
const CameraSection = ({
  ...
  return <div className="relative">
      <div className="relative bg-gray-900 rounded-full overflow-hidden w-80 h-96 mx-auto">
        {cameraError ? <div className="flex flex-col items-center justify-center h-full p-6 text-center">
            ...
            {stream && !isBiometricLogin && <div className="absolute inset-0 flex items-center justify-center">
                <div className={`relative w-48 h-48 border-2 rounded-full transition-colors duration-300 ${scanComplete ? 'border-green-400 shadow-lg shadow-green-400/50' : isScanning ? 'border-cyan-400 shadow-lg shadow-cyan-400/50 animate-pulse' : 'border-gray-400'}`}>
                  {/* Corner indicators */}
