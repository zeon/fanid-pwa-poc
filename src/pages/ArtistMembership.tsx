...
const ArtistMembership = () => {
  ...
  const { checkMembershipStatus } = useMembershipOperations();

  const artistIdNum = parseInt(artistId || '1');
  const isMember = checkMembershipStatus(artistIdNum);

  // Redirect to dashboard if user doesn't have membership
  if (!isMember) {
